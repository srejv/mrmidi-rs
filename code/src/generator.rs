...
// Declare variable (should be used)
fn dv() -> CodeNode {
...
pub fn test_program() {
    ...
    let set_variable = p.ac(sv("gl_FragCoord"), root);

    let f = p.ac(cf("vec4"), set_variable);
    for arg in &[v("1.0"), v("1.0"), v("0.0"), v("1.0")] {
        p.ac(arg.clone(), f);
    }
...
pub fn create_crt_fragment_shader() -> TreeProgram {
    ...
    p.ac(v("uv"), multiply);
    let subtract = p.ac(s(), multiply);
    p.ac(v("2.0"), subtract);
    p.ac(v("1.0"), subtract);

    let set_offset = p.ac(sv("vec2 offset"), root);
    let divide = p.ac(d(), set_offset);
    let f = p.ac(cf("abs"), divide);
    for arg in &[v("uv.yx")] {
        p.ac(arg.clone(), f);
    }

    let f = p.ac(cf("vec2"), divide);
    for arg in &[v("6.0"), v("4.0")] {
        p.ac(arg.clone(), f);
    }
    ...
    let declare_channel1 = p.ac(dv(), root);
    p.ac(v("vec3"), declare_channel1);
    let set_channel1 = p.ac(sv("channel1"), declare_channel1);
    let tex_call = p.ac(cf("texture2D"), set_channel1);
    p.ac(v("iChannel1"), tex_call);
    p.ac(v("mehuv"), tex_call);

    let declare_res = p.ac(dv(), root);
    p.ac(v("vec3"), declare_res);
    let set_res = p.ac(sv("res"), declare_res);
